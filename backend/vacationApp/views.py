...
from fastapi_service.database import listings_collection
...
def get_listings(request):
    ...
    try:
        listings = list(listings_collection.find().limit(10))  # limit to 10 results
        for listing in listings:
            listing["_id"] = str(listing["_id"])  # Make ObjectId JSON serializable
        return JsonResponse(listings, safe=False, status=200)
    except Exception as e:
        return JsonResponse({'error': f'Failed to fetch listings: {str(e)}'}, status=500)